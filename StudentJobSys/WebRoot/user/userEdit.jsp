	...
	<div align="left" style="margin-top: 5px;">
		<form action="UserServlet?method=save" id="edit" name="edit" method="post">
			<table border="0">
				...
				<tr>
					<c:if test="${not empty user.user_checkstatus }">
						<td>
							<input type="hidden" id="user_checkstatus" name="user_checkstatus" value="${user.user_checkstatus }">
						</td>
						<td>
					...
					<c:if test="${ empty user.user_checkstatus }">
						<td>
					...
					<c:otherwise>
						...
						<c:forEach items="${select_user_Type }" var="role">
							...
							<c:when test="${role.role_id ==user.user_type }">
								<option value=${user.user_type } checked>${role.role_name }</option>
							</c:when>
							<c:otherwise>
				...
				<tr>
					...
					<td class="tdrsname">
						...
						<input class="dfinput" type="text" id="user_desc" name="user_desc" value="${user.user_desc }" style="width: 300px;"> 
