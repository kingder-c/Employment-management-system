...
import com.weixin.teacher.service.TeacherService;
...
public class UserServlet extends BaseRequestUtil {
	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(user.getUser_type()==24L){
				company = companyservice.getById(user_desc);
			}
			if(user.getUser_type()==26L){
	...
	private void edit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("user/userEdit.jsp").forward(
				request, response);
